...
const rows = [
  ...
  row({ _id: 'f', status: 'removed', notice: 'not_sent', decision: 'removed' }),
]
...
describe('view presets', () => {
  ...
  it('all shows everything and is the only selectable view', () => {
    expect(applyFilters(rows, VIEWS.all.filters)).toHaveLength(6)
...
describe('applyFilters', () => {
  it('filters by branch, guardian and minimum sections', () => {
    ...
    expect(applyFilters(rows, { ...VIEWS.all.filters, minSections: 5 })).toHaveLength(5)
  })
  it('filters by notice state', () => {
    expect(applyFilters(rows, { ...VIEWS.all.filters, notice: 'not_sent' }).map((r) => r._id)).toEqual(['d', 'e', 'f'])
  })
})
...
describe('batchable', () => {
  ...
  it('excludes removals — that email sends itself the moment the decision is made', () => {
    expect(batchable(rows).map((r) => r._id)).not.toContain('f')
  })
})
